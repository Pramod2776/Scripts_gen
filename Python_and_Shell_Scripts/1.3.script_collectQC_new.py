...
def add_picard(table, names, dirs):
    ...
    for i in range(len(names)):
        ...
        for file in files:
            if file == md_file:
              md_files.append(join(dirs[i], md_file))
              count += 1
            elif file == is_file:
              is_files.append(join(dirs[i], is_file))
              count += 1
            elif file == rna_file:
              rna_files.append(join(dirs[i], rna_file))
              count += 1
            elif file == as_file:
              as_files.append(join(dirs[i], as_file))
              count += 1
            elif file == gc_file:
              gc_files.append(join(dirs[i], gc_file))
              count += 1
        if count != 5:
...
def add_rnaseqc(table, names, dirs):
    ...
    for i in range(len(names)):
        ...
        if 'metrics.tsv' not in listdir(join(dirs[i], folder2)):
            print('Error: missing RNA_SeQC files: ' + names[i])
            exit()
        files.append(join(dirs[i], folder2, 'metrics.tsv'))
    for file_index in range(len(names)):
        with open(files[file_index], 'r') as f:
            lines = f.read().splitlines()
